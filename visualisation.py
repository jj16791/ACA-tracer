    ...
    def redraw(self, first_index, last_index, view_width):
        ...
        for i in range(first_index, last_index):
            ...
            for ch in trace_data[i][1]:
                ...
                self.pad.addstr(index_y, index_x, ch, curses.color_pair(current_colour))
                index_x+=1
            current_colour = 0
            index_y+=1
                ...
                for j in range(0, len(probe_titles)):
                    if(selected_probes[j]):
                        if(probe_data[i][0][j] == "-"):
                            ...
                            self.pad.addstr(index_y,index_x,"-",curses.color_pair(0))
                        elif(probe_data[i][0][j] == "1"):
                            # For probes that occur once in the cycle
                            colour = probe_colour_mapping.index(probe_titles[j][1])+1
                            self.pad.addstr(index_y,index_x," ",curses.color_pair(colour))
                        elif(probe_data[i][0][j] != "."):
                            # For probes that occur mutliple times in the cycle
                            colour = probe_colour_mapping.index(probe_titles[j][1])+1
                            self.pad.addstr(index_y,index_x,probe_data[i][0][j],curses.color_pair(colour))
                        index_y+=1
                index_x+=1
                index_y=0
        super().redraw(view_width, width_left)
            ...
            for j in range(0, len(probe_titles)):
                if(selected_probes[j]):
                    ...
                    if (width>=self.cols):
                        self.cols = width+1
                        self.pad.resize(self.rows, self.cols)
                    self.pad.addstr(index_y,0,asct_insn_string[j],curses.color_pair(0))
                    index_y+=1
            index_y=0
        super().redraw(view_width, width)
...
def cacheLineOffsets(file, force, type):
    ...
    if os.path.exists(cache_filepath) and (not force):
        # Read
        num_file_items = int(os.path.getsize(cache_filepath)/line_offsets.itemsize)
...
def readTrace(file, index):
    ...
    line_existance=(trace_data[index] == (0,"","",0)) if index < len(trace_data) else False
    if(line_existance):
        ...
        if(line != ''):
            ...
            for i in range(1, len(trace_stages)-1):
                if(stages[i]!=0):
                    # If dispatch and issue occur on the same cycle display issue only
                    if(stages[i] == stages[i+1]):
                        continue
                    trace_string+=trace_stages[i]+((stages[i+1]-stages[i]-1)*".")
                else:
                    break
...
def readProbe(file, index):
    ...
    if(index<len(probe_line_offsets)):
        ...
        file.seek(0,2)
    line_existance=(probe_data[index][0][0] == ".") if index <= probe_file_length else False
    if(line_existance):
        line = file.readline()
        if(line=='-\n'):
            ...
            for probe in data:
                ...
                insertion = str(chosen[probe_index]) if chosen[probe_index]<10 else "+"
                # Change print string to include occurance of probe
                probe_string="".join((probe_data[index][0][:probe_index],insertion,probe_data[index][0][probe_index+1:]))
            ...
            probe_data[index]=(probe_data[index][0].replace('.','-'),probe_data[index][1])
        elif(line==''):
...
def calcInfoOffsets(file):
    ...
    info_seperations=[1,cycle_title_offset,pc_title_offset,disasm_title_offset]
    # Reset file pointer
    file.seek(prev_pos)
...
def visualiser(trace, probe):
    ...
    for i in range(1,len(probe_titles)):
        placeholder+=":0"
    probe_data=[(len(probe_titles)*'.',placeholder)]*(probe_file_length+1)
    ...
    try:
        ...
        if(x >= 49 and y >= (5+probe_length)):
            ...
            for i in range(top, bottom):
                readTrace(trace,i)
            for j in range(trace_data[top][0]-1, trace_data[top][0]+width_left+1):
                readProbe(probe,j)
            # Set column sizes based on width of current trace lines
            win_cols_left = width_left+3 if width_left+3 < (math.trunc(x/4)) else math.trunc(x/4)
            ...
            win2 = Window(win_rows_bot, win_cols_left, win_rows_top, 0, ["[PROBE]"])
            win3 = Window(win_rows_top, win_cols_right, 0, win_cols_left, ["[INSN_NUM]","[CYCLE]","[PC]","[DISASM]"])
            win4 = Window(win_rows_bot, title_cols, win_rows_top, win_cols_left, ["[PROBES SELECTED]"])
            win5 = Window(win_rows_bot, asct_insn_cols, win_rows_top, win_cols_left+title_cols, ["[ASSOCIATED_INSTRUCTIONS]"])
            # Create pad classes and draw initial trace/probe data
            pad1 = TracePad(pad_top_rows, pad_left_cols, 0, 0, 1, 1, win_rows_top-2, win_cols_left-2)
            pad1.redraw(top, bottom, win_cols_left-2)
            pad2 = ProbePad(pad_bot_rows+1, pad_left_cols, 0, 0, win_rows_top+1, 1, y-2, win_cols_left-2)
            pad2.redraw(top, win_cols_left-2)
            pad3 = InfoPad(pad_top_rows, win_cols_right-2, 0, 0, 1, win_cols_left+1, win_rows_top-2, x-2)
            pad3.redraw(top, bottom, win_cols_right-2)
            pad4 = TitlePad(pad_bot_rows, title_cols-2, 0, 0, win_rows_top+1, win_cols_left+1, y-2, win_cols_left+title_cols-2)
            pad4.redraw(title_cols-2)
            pad5 = AssociatedPad(pad_bot_rows, asct_insn_cols-2, 0, 0, win_rows_top+1, win_cols_left+title_cols+1, y-2, x-2)
            ...
            while(running):
                ...
                if k == curses.KEY_RESIZE:
                    ...
                    if top>0:
                        ...
                        if (((bottom-top)==(pad_top_rows)) or ((bottom-top)==trace_file_length)):
                            bottom-=1
                        top-=1
                    ...
                    # Scroll right
                    pad1.x_pos+= 1 if (pad1.x_pos+win_cols_left-2)<width_left else 0;
                    pad2.x_pos+= 1 if (pad2.x_pos+win_cols_left-2)<width_left else 0;
                    pad3.x_pos+= 1 if (pad3.x_pos+win_cols_right-2)<pad3.cols-1 else 0;
                    pad4.x_pos+= 1 if (pad4.x_pos+title_cols-2)<pad4.cols-1 else 0;
                    pad5.x_pos+= 1 if (pad5.x_pos+asct_insn_cols-2)<pad5.cols-1 else 0;
                    ...
                    y, x = screen.getmaxyx()
                    winTemp = Window(3, math.trunc(x/3),math.trunc(y/2)-1,math.trunc(x/3),["[JUMP_TO]"])
                    typing = True
                    input=""
                    # Update screen
                    curses.doupdate()
                    ...
                    while(typing):
                        ...
                        if(len(input)>(winTemp.cols-2)):
                            winTemp.win.addstr(1,1,input[(len(input)-winTemp.cols+2):])
                        else:
                    ...
                    winTemp_rows=len(probe_titles)+2 if math.trunc(y/2) >= len(probe_titles)+2 else math.trunc(y/2)
                    winTemp_cols=math.trunc(x/2)
                    winTemp = Window(winTemp_rows,winTemp_cols,math.trunc(y/4),math.trunc(x/4),["[PROBE_SELECTION]"])
                    ...
                    padTemp = SelectionPad(padTemp_rows,padTemp_cols,0,0,start_y_loc,start_x_loc,end_y_loc,end_x_loc)
                    ...
                    while(selecting):
                        ...
                        if(chr(l)=='z' or chr(l)=='Z'):
                            ...
                            if (index>=winTemp.rows-2):
                                padTemp.y_pos+=1 if padTemp.y_pos<=(padTemp.rows-winTemp.rows+1) else 0
                            ...
                            padTemp.x_pos+=1 if (padTemp.x_pos+winTemp.cols-2)<padTemp.cols-1 else 0;
                        elif(l==10):
                            # Toggle probe
                            if selected_probes[index]==1:
                                ...
                                probes_active+=1 if probes_active<len(probe_titles) else 0
                ...
                if not queued:
                    ...
                    win2.reset_resize(win_rows_bot, win_cols_left, win_rows_top, 0)
                    win3.reset_resize(win_rows_top, win_cols_right, 0, win_cols_left)
                    win4.reset_resize(win_rows_bot, title_cols, win_rows_top, win_cols_left)
                    win5.reset_resize(win_rows_bot, asct_insn_cols, win_rows_top, win_cols_left+title_cols)
                    # Redraw pads
                    pad1.reset_resize(pad_top_rows, pad_left_cols, 1, 1, win_rows_top-2,
                                        win_cols_left-2)
                    pad1.redraw(top, bottom, win_cols_left-2)
                    pad2.reset_resize(pad_bot_rows+1, pad_left_cols, win_rows_top+1, 1, y-2,
                                        win_cols_left-2)
                    pad2.redraw(top, win_cols_left-2)
                    pad3.reset_resize(pad_top_rows, win_cols_right-2, 1, win_cols_left+1,
                                        win_rows_top-2, x-2)
                    pad3.redraw(top, bottom, win_cols_right-2)
                    pad4.reset_resize(pad_bot_rows, title_cols-2, win_rows_top+1,
                                        win_cols_left+1, y-2, win_cols_left+title_cols-2)
                    pad4.redraw(title_cols-2)
                    pad5.reset_resize(pad_bot_rows, asct_insn_cols-2, win_rows_top+1,
...
def main():
    ...
    parser.add_argument('--force', action='store_true', default=False, help='Force the rewrite of .cache files')
